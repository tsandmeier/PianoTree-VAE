...
# Initialize project
###############################################################################
ENTROPY_LEVEL = project_params['entropy_level']
PROJECT_NAME = project_params['project_name'] + "_" + ENTROPY_LEVEL
MODEL_PATH = os.path.join(project_params['model_path'], ENTROPY_LEVEL)
LOG_PATH = os.path.join(project_params['log_path'], ENTROPY_LEVEL)
...
if ENTROPY_LEVEL == "low_entropy":
    entr = 0
elif ENTROPY_LEVEL == "mid_entropy":
    entr = 1
elif ENTROPY_LEVEL == "high_entropy":
    entr = 2

training_data = ambroseDataset.AmbroseDataset(dataset_path, fraction=0.95, entropy=entr)
val_data = ambroseDataset.AmbroseDataset(dataset_path, fraction=0.05, train=False, entropy=entr)
...
def train(model, train_loader,
    ...
    for i, batch in enumerate(train_loader):
        ...
        print(f"Batch {i}/{len(train_loader)}", '\r')
        sys.stdout.flush()
